...
export default function AboutSection() {
  ...
  return (
    <section
      id="about"
      className="relative min-h-screen w-full flex flex-col items-center justify-center bg-[#0C0C0C] py-20 px-5 sm:px-8 md:px-10 overflow-hidden"
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center max-w-[800px] w-full">
        ...
        <FadeIn delay={0} y={40} duration={0.7}>
          <h2 className="hero-heading font-black uppercase leading-none tracking-tight text-center text-[3rem] sm:text-[6rem] md:text-[8rem] lg:text-[10rem] select-none" style={{ fontSize: "clamp(3rem, 12vw, 160px)" }}>
            About me
          </h2>
        </FadeIn>

        {/* Spacer using gap sizing described: gap-10 sm:gap-14 md:gap-16 */}
        <div className="h-10 sm:h-14 md:h-16 w-full" />

        {/* Paragraph Text with character reveal */}
        <AnimatedText
          text="With more than five years of experience in design, i focus on branding, web design, and user experience, i truly enjoy working with businesses that aim to stand out and present their best image. Let's build something incredible together!"
          className="text-[#D7E2EA] font-medium leading-relaxed max-w-[560px] text-center"
        />

        {/* Spacer using gap sizing described: gap-16 sm:gap-20 md:gap-24 */}
        <div className="h-16 sm:h-20 md:h-24 w-full" />

        {/* Contact button */}
        <div>
          <ContactButton onClick={() => handleScrollTo("contact-form-section")} />
        </div>
      </div>
    </section>
  );
